c************************************************************************************
c             Download of sourcecode iftc.f
c************************************************************************************
c
c   1) Compilation (linux):  gfortran -m32 -O2 iftc.f -o iftc
c
c   2) Run:                  iftc < inputfile.d                    
c
c   3) The file: inputfile.d has to contain the 13 lines:
c
...
c   line 7:  value for ratio            or a blank line
c   line 8:  value for method           or a blank line
c   line 9:  value for no of points     or a blank line
c   line 10: value for no of extra calc or a blank line
c   line 11: value for transformation   or a blank line
c   line 12: value for background       or a blank line
c   line 13: value for screensize       or a blank line
c
...
C*********************************************************************************
      PROGRAM IFTc
C********************************************************************************
C     Various parameters may be changed to improve the convergence/result.
C    
C     In the successive over/under-relaxation algorithm in FUNC the
C     parameters OMEGA and MAXIT determines the steplength of each iteration
C     and the maximum number of iterations respectively. Both parameters may
C     be changed. Search for the line: Change omega and maxit here:
C     The criterion for the convergence of the same algorithm XPREC (which
C     is the cosine of the angle between the gradient of the constraint and
C     the chi-square) may be reduced from 0.995 for shorter CPU-times. 
C     The relevant line to be changed is found next to the line determining OMEGA
C
C     The constraints for S(q) may be changed to obtain S(q) -> 1 for large q.
C     Search for the line: Change the constraints for S(q) here:
C********************************************************************************
      PARAMETER (NMAX=1000)
      ...
      common /data4/ mtot
      common /data3/ x,sd,y,xf,ntot,prob,ftot,nof
      ...
      CHARACTER*6 CHAR
      CHARACTER*1 ANSWER,answer2,answer5,answer8,answer7
      character*30 dummy 
      character*80 string
      real pi
C********************************************************************
...
c     ((the axial ratio cannot be fitted by evidence without an excluded volume ddf.))
  ...
  139 FORMAT(1X,'Number of points used for p(r)    (50-100)     => ',$)
      ...
      if((etaest.gt.0).and.(ntot.gt.80)) cpumax=90
...
c********************************************************************
 ...
 6661 continue
      ...
      ftol=rprec
      ngrid=1
      if(alphaest.eq.-100) call alpha_e(p,ndtest)
      ngrid=0
...
c****************************************
  ...
  919 FORMAT(1X,'Parameters in         :       ',A)
      ...
      x(1)=x(nn+1)
      call plot(xf(ptot),f,ptot,fm,mtot,nadd,aname,x,cav,alp,dot)
...
c*****************************************************************************
      ...
      character*1 answer,answer2,answer5
      ...
      if(ratio.le.0.1) ratio=0.1+ratio/1000
      if(ratio.ge.10) ratio=10-ratio/1000

      dfx=diam/ntot
      dmax=diam
c
c     Change omega and maxit here:
c
c     (niter being the number of iterations in powell)
      omega=0.5
...
c     sigma(j)=1
      if((answer5.eq.'K').or.(answer5.eq.'N')) then
      sigma(j)=1
      goto 63
      endif
...
c********************************************************
 ...
 5000  continue 
c*********************************************************
c unity matrix for entropy regularization
c does not contribute to evidence
c
c      do 2001 i=1,ntot
c      do 2097 j=1,ntot
c      u(i,j)=0
c      if(abs(i-j).eq.0) u(i,j)=u(i,j)+1.
c 2097 continue
c 2001 continue 
c
c      call SVDCMP(u,ntot,Ntot,nmax,Nmax,W,smear)
c
c 
c 2013 rnorm=0
c
c      do 2002 i=1,ntot
c      rnorm=rnorm+log(abs(w(i)))  
c 2002 continue
      rnorm=0
...
c does not contribute to evidence
c
c      do 2021 k1=ntot+ntot2+1,ntot+nextra
c      do 2087 k2=ntot+ntot2+1,ntot+nextra
c      i=k1-ntot-ntot2
c      j=k2-ntot-ntot2
c      u(i,j)=0
c      if(abs(i-j).eq.0) u(i,j)=u(i,j)+1
...
c 2021 continue 
c
c      call SVDCMP(u,ntotx,Ntotx,nmax,Nmax,W,smear)
c
c      do 2032 i=1,ntotx
c      rnorm=rnorm+log(abs(w(i)))  
c 2032 continue
c******************************************************************
c     evidence calculated here:
c******************************************************************

 2228  evidence=0.5*rnorm-log(abs(dmax))+
     -(alpha*S-0.5*c*mtot)-0.5*rlogdet-log(abs(alpha))
      if(eta.gt.0.00001) evidence=evidence-log(abs(eta))
c******************************************************************
c******************************************************************
  ...
  161 FORMAT(1X,I4,1x,f6.2,1X,f10.4,
     -1X,F10.4,1x,f7.3,2x,f7.4,1x,f8.4,1x,f10.1)
  180 WRITE(6,161)ite,dmax,log(abs(ALPHA))
c     -,c,ratio,DOTSP,eta,evidence
     -,c,ratio,DOTSP,f(0)*0.1*(xf(2)-xf(1)),evidence
...
c      write(6,*)f(0)*0.1*(xf(2)-xf(1)),
c    -ftot(0,nof)*0.1*ftot(nmax-1,nof)/ntot
...
c      write(6,*)'ratio = ',ratio,t

 9998 if(ratio.le.0.1) ratio = 0.1
      if(ratio.ge.10) ratio = 10
...
c reskalering af xf, s\E5 max r = 4 (max radius = 1)
      r=2.*xf(j)/diam
        if(r.le.2) then
...
c fra ovenst. f\E5s gamma(r) normeret til gamma(0)=1 og gamma(2*diam)=0
  10  continue
c      write(150,*)xf(j),c,j,ntot,xf(101)     
      m(j)=-c*xf(j)**2
c      write(151,*)xf(j),m(j),j,ntot,xf(101)     
...
c**********************************************************************
      subroutine plot(dmax,f,ptot,fm,mtot,nadd,oname,x,chi,alpha,dotsp)
      parameter (nmax=1000)
      CHARACTER*18 oname
      character*10 aname
...
c      CHARACTER*23 hxname,gsname,gxname,sname
      ...
      character*6 hxname
      character*1 answer8,answer5,answer7
      ...
      common /back/ background
      common /screen/ answer8,answer7
      real xf(0:nmax),f(0:nmax),fm(nmax),x(nmax)
      plname='pl'//aname
      aname=oname
...
c      sname='st_'//bname
  ...
  12  continue
      ...
      if(dotsp.ge.0.9) comment='Conv. OK'

      if(dotsp.le.0.9) comment='NOT OK !'
...
c changed 6.2011*************'
c      write(88,544)aname,chi,alpha,comment
c 544  format(1x,'set title "p(r) ',
c     -'',a,' chi = ',f6.3,' log(alpha) =',f6.1,
c     -' ',a,'"')
